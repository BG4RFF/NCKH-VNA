...
{
    // le = 0;
    GPIOPinWrite(ADF_GPIO_PORT_BASE, ADF_LE_PIN, 0);

    for (int i = 3; i >= 0; i--) // loop on 4 x 8 bits
    {
        SoftwareSPI_SendData((value >> 8 * i) & 0xFF); // offset, byte masking and sending via SPI
    }

    // le = 1;
    GPIOPinWrite(ADF_GPIO_PORT_BASE, ADF_LE_PIN, ADF_LE_PIN);
...
void ADF4350_Init(void)
{
    ...
    GPIOPinTypeGPIOOutput(ADF_GPIO_PORT_BASE, ADF_CE_PIN | ADF_LE_PIN);
    GPIOPinWrite(ADF_GPIO_PORT_BASE, ADF_CE_PIN | ADF_LE_PIN, ADF_CE_PIN | ADF_LE_PIN);
